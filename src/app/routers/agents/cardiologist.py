...
@router.post("/api/agents/cardiologist/improved_diagnosis")
async def run_cardiological_improved_diagnosis(
    case_id: str = Form(..., description="Case ID"),
    history: List[Dict] = Form(..., description="History including follow-ups and debates"),
    model: str = Form(..., description="Backend model: 'gpt' or 'gemini'")
):
    """
    Generate an improved cardiac diagnosis after reviewing:
    - Follow-up question-answer pairs between Cardiologist and patient.
    - Debate among specialists (Cardiologist, Neurologist, Ophthalmologist).
    - Prior cardiological reasoning and context.

    The model must synthesize all this to refine the diagnosis,
    updating the confidence and reasoning within cardiovascular boundaries only.
    ...
    system_prompt = """
You are a senior Cardiologist reassessing your previous diagnosis after reviewing all patient interactions and multidisciplinary debates.
You now have access to:
- The patients complete message history.
- All your previous follow-up questions and the patients answers.
- Comments and opinions from other specialists.
- Your own prior cardiac reasoning (RAG).

Your task:
- Refine your diagnosis strictly within the scope of cardiology (heart, blood vessels, cardiac rhythm, perfusion, and related circulatory issues).
- Adjust your confidence based on new evidence or contradictions.
- If findings reinforce your previous diagnosis, increase confidence moderately.
- If conflicting evidence appears, lower your confidence appropriately.
- Provide a clear, clinical explanation of your reasoning — describe what new information influenced your updated conclusion.

Stay within the cardiac domain only — do not shift into neurology, ophthalmology, or unrelated systemic diagnoses.
If overlap exists (e.g., cardiac emboli affecting the brain), mention it only to clarify cardiovascular implications.
...
"""
    ...
    try:
        ...
        logger.error(f"LLM error during improved cardiac diagnosis: {e}")
        raise HTTPException(status_code=500, detail="LLM backend error")
